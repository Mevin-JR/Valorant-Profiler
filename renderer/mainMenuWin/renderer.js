...
async function bruh() {
    try {
        const response = await fetch(`https://api.henrikdev.xyz/valorant/v1/account/supposedlysam/HSC`, {
          method: 'GET',
          headers: {
...
function loadHome() {
    ...
    account.getUserProfiles(`?t=${timestamp}`)
    .then((userProfiles) => {
        setCards(userProfiles);

        const accountRank = document.querySelectorAll('.account-rank');
        accountRank.forEach((account) => {
            const rankText = account.textContent;
            if (rankText.includes('Iron')) {
                account.style.color = '#FDFDFD';
                account.style.backgroundColor = '#787775';
            } else if (rankText.includes('Bronze')) {
                account.style.color = '#54370A';
                account.style.backgroundColor = '#A58642';
            } else if (rankText.includes('Silver')) {
                account.style.color = '#FDFDFD';
                account.style.backgroundColor = '#464B4E';
            } else if (rankText.includes('Gold')) {
                account.style.color = '#F4EEAE';
                account.style.backgroundColor = '#CC8818';
            } else if (rankText.includes('Platinum')) {
                account.style.color = '#29747F';
                account.style.backgroundColor = '#52CDDD';
            } else if (rankText.includes('Diamond')) {
                account.style.color = '#F195F4';
                account.style.backgroundColor = '#946BD2';
            } else if (rankText.includes('Ascendant')) {
                account.style.color = '#B6FFD7';
                account.style.backgroundColor = '#228052';
            } else if (rankText.includes('Immortal')) {
                account.style.color = '#E8C3A8';
                account.style.backgroundColor = '#C3324E';
            } else if (rankText.includes('Radiant')) {
                account.style.color = '#E39C41';
                account.style.backgroundColor = '#FFFFB4';
            }
        })
    });
